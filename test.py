...
def detect_red_hat(frame, lower_red, upper_red, area_min, area_max):
    ...
    for cnt in contours:
        ...
        if area_min <= area <= area_max:  # Check area thresholds
            ...
            cv2.circle(frame, (cx, cy), 5, (0, 0, 255), -1)  # Always draw the red dot
...
time.sleep(8)  # Wait for the drone to reach ~2.5m altitude
...
try:
    ...
    while True:
        ...
        if h_min and h_max and s_min and s_max and v_min and v_max:
            ...
            previous_suitcase_pos = None  # Store the last known bounding box for the suitcase

            # Inside the main loop
            for result in results:
                for box in result.boxes:
                    # Check for Suitcase Detection
                    if int(box.cls[0]) == SUITCASE_CLASS_ID and box.conf[0] > 0.05:
                    ...
                    elif previous_suitcase_pos:
                        x1, y1, x2, y2 = previous_suitcase_pos
                        cx = (x1 + x2) // 2
                        cy = (y1 + y2) // 2
            ...
            current_time = time.time()
            if person_pos and chair_pos and current_time - last_path_time > skip_duration:
                print(f"Source: {person_pos}, Target: {chair_pos}. Calling path_calculation.py...")
                frame_filename = os.path.join(frames_dir, f"frame_{int(time.time())}.jpg")
                cv2.imwrite(frame_filename, frame)  # Save the frame
                print(f"Frame saved at: {frame_filename}")

                # Call the subprocess for path calculation
                subprocess.Popen(
                    ["C:/Users/97254/PycharmProjects/Final_Project/venv/Scripts/python.exe", "path_calculation.py",
                     frame_filename,
                     str(person_pos[0]), str(person_pos[1]),  # Pass person coordinates
                     str(chair_pos[0]), str(chair_pos[1])]  # Pass chair coordinates
                )
                last_path_time = current_time  # Update timer
